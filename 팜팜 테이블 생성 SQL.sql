...
COMMENT ON COLUMN "ORDER_PAYMENT"."ORDER_NO" IS '주문 번호(SEQ_ORDER_NO)';

COMMENT ON COLUMN "ORDER_PAYMENT"."IMP_UID" IS '결제 UID';
...
COMMENT ON COLUMN "ORDER_SHIPPING"."ORDER_NO" IS '주문 번호(SEQ_ORDER_NO)';

COMMENT ON COLUMN "ORDER_SHIPPING"."INVOICE_NO" IS '주문 송장번호';

ALTER TABLE "ORDER_SHIPPING" ADD CONSTRAINT "PK_ORDER_SHIPPING" PRIMARY KEY (
	"ORDER_NO"
);

ALTER TABLE "ORDER_SHIPPING" ADD CONSTRAINT "FK_ORDER_TO_ORDER_SHIPPING_1" FOREIGN KEY (
	"ORDER_NO"
)
REFERENCES "ORDER" (
	"ORDER_NO"
...
-- 1:1 상담 테이블
CREATE TABLE "INQUIRE_ROOM" (
	"INQUIRE_NO"	NUMBER		NOT NULL,
	"INQUIRE_DATE"	DATE	DEFAULT SYSDATE	NOT NULL,
	"MEMBER_NO"	NUMBER		NOT NULL,
	"MEMBER_NO2"	NUMBER		NOT NULL,
	"CLOSED_FL"	CHAR(1)	DEFAULT 'N'	NOT NULL
);

COMMENT ON COLUMN "INQUIRE_ROOM"."INQUIRE_NO" IS '1:1 상담 번호(SEQ_INQUIRE_NO)';

COMMENT ON COLUMN "INQUIRE_ROOM"."INQUIRE_DATE" IS '채팅방 개설일';

COMMENT ON COLUMN "INQUIRE_ROOM"."MEMBER_NO" IS '개설자 회원번호';

COMMENT ON COLUMN "INQUIRE_ROOM"."MEMBER_NO2" IS '참여자 회원번호';

COMMENT ON COLUMN "INQUIRE_ROOM"."CLOSED_FL" IS '방 폐쇄 여부';

CREATE TABLE "INQUIRE_IMG" (
	"CHAT_IMG_NO"	NUMBER		NOT NULL,
	"CHAT_IMG_ORIGINAL"	VARCHAR2(500)		NOT NULL,
	"CHAT_IMG_RENAME"	VARCHAR2(50)		NOT NULL,
	"CHAT_IMG_PATH"	VARCHAR2(500)		NOT NULL,
	"CHAT_IMG_DEL_FL"	CHAR(1)	DEFAULT 'N'	NOT NULL,
	"CHAT_NO"	NUMBER		NOT NULL,
	"Field"	VARCHAR(255)		NULL
);

COMMENT ON COLUMN "INQUIRE_IMG"."CHAT_IMG_NO" IS '메세지 이미지 번호(SEQ_MESSAGE_IMG_NO)';

COMMENT ON COLUMN "INQUIRE_IMG"."CHAT_IMG_ORIGINAL" IS '본래 이름';

COMMENT ON COLUMN "INQUIRE_IMG"."CHAT_IMG_RENAME" IS '수정 후 이름';

COMMENT ON COLUMN "INQUIRE_IMG"."CHAT_IMG_PATH" IS '사진 경로';

COMMENT ON COLUMN "INQUIRE_IMG"."CHAT_IMG_DEL_FL" IS '삭제 여부';

COMMENT ON COLUMN "INQUIRE_IMG"."CHAT_NO" IS '메세지 번호(SEQ_MESSAGE_NO)';
...
CREATE TABLE "MESSAGE" (
	...
	"IMG_FL"	CHAR(1)	DEFAULT 'N'	NOT NULL,
	"ROOM_NO"	NUMBER		NOT NULL
);
...
COMMENT ON COLUMN "MESSAGE"."MESSAGE_DEL_FL" IS '메세지 삭제 여부';

COMMENT ON COLUMN "MESSAGE"."READ_FL" IS '메시지 확인 여부';

COMMENT ON COLUMN "MESSAGE"."MESSAGE_TIME" IS '메세지를 발신한 시간';

COMMENT ON COLUMN "MESSAGE"."IMG_FL" IS '사진 여부';

COMMENT ON COLUMN "MESSAGE"."ROOM_NO" IS '1:1 상담 번호(SEQ_INQUIRE_NO)';

ALTER TABLE "INQUIRE_ROOM" ADD CONSTRAINT "PK_INQUIRE_ROOM" PRIMARY KEY (
	"INQUIRE_NO"
);

ALTER TABLE "INQUIRE_IMG" ADD CONSTRAINT "PK_INQUIRE_IMG" PRIMARY KEY (
	"CHAT_IMG_NO"
);
...
ALTER TABLE "INQUIRE_ROOM" ADD CONSTRAINT "FK_MEMBER_TO_INQUIRE_ROOM_1" FOREIGN KEY (
	"MEMBER_NO"
)
REFERENCES "MEMBER" (
	"MEMBER_NO"
...
ALTER TABLE "INQUIRE_ROOM" ADD CONSTRAINT "FK_MEMBER_TO_INQUIRE_ROOM_2" FOREIGN KEY (
	"MEMBER_NO2"
)
REFERENCES "MEMBER" (
	"MEMBER_NO"
...
ALTER TABLE "INQUIRE_IMG" ADD CONSTRAINT "FK_MESSAGE_TO_INQUIRE_IMG_1" FOREIGN KEY (
	"CHAT_NO"
)
REFERENCES "MESSAGE" (
	"MESSAGE_NO"
...
ALTER TABLE "MESSAGE" ADD CONSTRAINT "FK_INQUIRE_ROOM_TO_MESSAGE_1" FOREIGN KEY (
	"ROOM_NO"
)
REFERENCES "INQUIRE_ROOM" (
	"INQUIRE_NO"
);
...
CREATE SEQUENCE SEQ_MESSAGE_IMG_NO NOCACHE;
