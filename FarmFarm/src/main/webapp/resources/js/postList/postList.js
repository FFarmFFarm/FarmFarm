...
/* keyword와 category, cp 정보를 전달받아 처리하는 ajax */
...
const makeHistory1 = (category, cp, sort, exceptFl) => {
    ...
    const title = '';
    const url = '/post/list?' + 'category=' + category + '&cp=' + cp + '&sort=' + sort + "&exceptFl=" + exceptFl;
...
const makeHistory2 = (keyword, category, cp, sort, exceptFl) => {
    ...
    const title = '';
    const url = '/post/list?' + 'keyword=' + keyword + '&category=' + category + '&cp=' + cp + '&sort=' + sort + "&exceptFl=" + exceptFl;
...
const doSearch = () => {
    ...
    if (keyword.trim().length != 0) {
        getCustomList2(keyword, 0, 1, sort, exceptFl);
